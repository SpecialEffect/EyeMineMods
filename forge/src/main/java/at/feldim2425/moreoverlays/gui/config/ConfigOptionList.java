...
import at.feldim2425.moreoverlays.gui.config.ConfigOptionList.OptionEntry;
...
// TODO: As I wrote this system I noticed, that the way AbstractSelectionList renders items and passes events is not optimal for this purpose
...
public class ConfigOptionList extends AbstractSelectionList<OptionEntry> {
    ...
    public CommentedConfig comments;
    
	public static Logger LOGGER = LogManager.getLogger("ConfigOptionList");
    ...
    @Override
    protected void renderDecorations(PoseStack poseStack, int p_renderDecorations_1_, int p_renderDecorations_2_) {
        int i = this.getItemCount();
        for(int j = 0; j < i; ++j) {
            int k = this.getRowTop(j);
            int l = this.getRowTop(j) + ITEM_HEIGHT;
            if (l >= this.y0 && k <= this.y1) {
                OptionEntry e = this.getEntry(j);
                e.runRenderTooltip(poseStack);
            }
        }
    }
    ...
    public void setConfiguration(ForgeConfigSpec rootConfig, List<String> path) {
        ...
        try {
            ...
            LOGGER.warn("Couldn't reflect childConfig from ForgeConfigSpec! Comments will be missing.", e);
        }
        ...
        if (val instanceof UnmodifiableConfig) {
        	...
        	if (n > 1) {
        		if (path.get(n-1) == path.get(n-2)) {
        			LOGGER.error("Attempting to load duplicate path:", path);
                    LOGGER.warn("This could be caused by key event race condition");
                    // Trim and reload
                    path.remove(n-1);
                	this.setPath(path);
                	return;
    ...
    public void refreshEntries() {
        ...
        for (final Map.Entry<String, Object> cEntry : this.currentMap.entrySet()) {
            ...
            if (cEntry.getValue() instanceof UnmodifiableConfig) {
            	final String key = categoryTitleKey(fullPath);
                String name = I18n.get(key);
                if (name.equals(key) && comment != null) { // translation failed, fallback to comment
                	name = comment;
                }
    ...
    public abstract static class OptionEntry extends AbstractSelectionList.Entry<OptionEntry> implements ContainerEventHandler {
        ...
        @Override
        public void render(PoseStack poseStack, int itemindex, int rowTop, int rowLeft, int rowWidth, int itemHeight, int mouseX, int mouseY,
            ...
            mouseX -= rowLeft;
            mouseY -= rowTop;
            poseStack.translate(rowLeft, rowTop, 0);
            renderControls(poseStack, rowTop, rowLeft, rowWidth, itemHeight, mouseX, mouseY, mouseOver, partialTick);

            poseStack.translate(-rowLeft, -rowTop, 0);
        }

        protected abstract void renderControls(PoseStack poseStack, int rowTop, int rowLeft, int rowWidth, int itemHeight, int mouseX, int mouseY,
        boolean mouseOver, float partialTick);
         ...
         * Not the best way but AbstractSelectionList doesn't seem to have any better hooks to do that.
         * A custom Implementation would be better but I'm too lazy to do that
         */
        public void runRenderTooltip(PoseStack poseStack){
            if(this.mouseOver){
                this.renderTooltip(poseStack, this.rowTop, this.rowLeft, this.rowWidth, this.itemHeight, this.mouseX, this.mouseY);
                Lighting.turnOff();
                GlStateManager._disableLighting();
            }
        }

        protected void renderTooltip(PoseStack stack, int rowTop, int rowLeft, int rowWidth, int itemHeight,int mouseX, int mouseY){
        }
